...
window.isSingleClickAllowed = false;
...
$("input[name='optradio']").change(function(){
    ...
    if ($(this).val() === 'lanes') {
        ...
        window.isAssistanceEnabled = false;
        window.isSingleClickAllowed = true;
    }
});
...
$(document).mousedown(function(event) {
    ...
    if(window.isEditMode &&  !window.isSingleClickAllowed && event.which === 1 &&  event.target.nodeName === "TD"){
        if (window.isMouseDown) {
            $(event.target).toggleClass("highlighted");
        }
    }else if(window.isEditMode &&  window.isSingleClickAllowed && event.which === 1 &&  event.target.nodeName === "TD"){
        console.log("marking seats....");
    }
    ...
    if(window.isEditMode &&  event.which === 1 && event.target.nodeName === "TD"){
        ...
        if(window.isSingleClickAllowed){
            $(event.target).toggleClass("highlighted");
            $(event.target).toggleClass("seat");
        }
    }
});
...
$('#saveMapBtn').click(function () {
    ...
    let nodes = [];
    let el_arr1 = $(".highlighted");
    //console.log(el_arr1.length);
    let el_arr2 = $(".seat");
    //console.log(el_arr2.length);


    for (let item of el_arr1) {
        nodes.push(item.id)
    }
    for (let item1 of el_arr2) {

        if(!nodes.includes(item1.id)){
            nodes.push(item1.id);
        }
    }
